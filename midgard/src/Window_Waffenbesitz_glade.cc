// generated 2002/5/13 20:33:57 CEST by thoma@Tiger.(none)
// using glademm V0.6.4b_cvs
//
// DO NOT EDIT THIS FILE ! It was created using
// glade-- /home/thoma/rollenspiel/midgard/programme/gcc_3.0/midgard/midgard.glade
// for gtk 1.2.10 and gtkmm 1.2.8
//
// Please modify the corresponding derived classes in ./src/Window_Waffenbesitz.cc

#include "config.h"
#include "Window_Waffenbesitz.hh"
#include <gdk/gdkkeysyms.h>
#include <gtk--/accelgroup.h>
#include <gtk--/label.h>
#include <gtk--/pixmap.h>
#include "../pixmaps/Weapon-trans-50.xpm"
#include <gtk--/viewport.h>
#include <gtk--/scrolledwindow.h>
#include "../pixmaps/Waf_Mag_bon-50.xpm"
#include <gtk--/box.h>
#include <gtk--/table.h>
#include <gtk--/adjustment.h>
#include "../pixmaps/Claymore-50.xpm"
#include <gtk--/button.h>

Window_Waffenbesitz_glade::Window_Waffenbesitz_glade(
) : Gtk::Window(GTK_WINDOW_TOPLEVEL)
{  
   
   Gtk::Window *Window_Waffenbesitz = this;
   Gtk::AccelGroup *Window_Waffenbesitz_accgrp = Gtk::AccelGroup::create();
   _data = new GlademmData(Window_Waffenbesitz_accgrp);
   
   Gtk::Label *label315 = manage(new class Gtk::Label("Besitz"));
   Gtk::Label *label316 = manage(new class Gtk::Label("Erwerb"));
   Gtk::Pixmap *pixmap9 = manage(new class Gtk::Pixmap(Weapon_trans_50_xpm));
   waffenbesitz_alt_tree = new class SimpleTree(5);
   
   Gtk::Viewport *viewport20 = manage(new class Gtk::Viewport());
   Gtk::ScrolledWindow *scrolledwindow56 = manage(new class Gtk::ScrolledWindow());
   Gtk::Pixmap *pixmap141 = manage(new class Gtk::Pixmap(Waf_Mag_bon_50_xpm));
   Gtk::Label *label561 = manage(new class Gtk::Label("Magischen Bonus\n"
		"eingeben"));
   Gtk::HBox *hbox65 = manage(new class Gtk::HBox(false, 0));
   checkbutton_mag_waffenbonus = manage(new class Gtk::CheckButton());
   
   Gtk::Table *table70 = manage(new class Gtk::Table(1, 2, false));
   waffenbesitz_neu_tree = new class SimpleTree(5);
   
   Gtk::Viewport *viewport21 = manage(new class Gtk::Viewport());
   Gtk::ScrolledWindow *scrolledwindow57 = manage(new class Gtk::ScrolledWindow());
   Gtk::Adjustment *spinbutton_av_bonus_adj = manage(new class Gtk::Adjustment(0, -5, 5, 1, 10, 10));
   spinbutton_av_bonus = manage(new class Gtk::SpinButton(*spinbutton_av_bonus_adj, 1, 0));
   
   Gtk::Adjustment *spinbutton_sl_bonus_adj = manage(new class Gtk::Adjustment(0, -5, 5, 1, 10, 10));
   spinbutton_sl_bonus = manage(new class Gtk::SpinButton(*spinbutton_sl_bonus_adj, 1, 0));
   label_waffe = manage(new class Gtk::Label("Waffe"));
   label_SL_bonus = manage(new class Gtk::Label("SLbonus"));
   label_av_bonus = manage(new class Gtk::Label("AVbonus"));
   
   Gtk::Label *label395 = manage(new class Gtk::Label("Kurzbeschreibung"));
   entry_magisch = manage(new class Gtk::Entry());
   
   Gtk::Table *table62 = manage(new class Gtk::Table(1, 2, false));
   table_magbonus = manage(new class Gtk::Table(4, 2, false));
   
   Gtk::Pixmap *pixmap155 = manage(new class Gtk::Pixmap(Claymore_50_xpm));
   Gtk::Label *label581 = manage(new class Gtk::Label("Zurück"));
   Gtk::HBox *hbox75 = manage(new class Gtk::HBox(false, 0));
   Gtk::Button *button58 = manage(new class Gtk::Button());
   Gtk::Table *table43 = manage(new class Gtk::Table(4, 3, false));
   label315->set_alignment(0, 0.5);
   label315->set_padding(0, 0);
   label316->set_alignment(0, 0.5);
   label316->set_padding(0, 0);
   pixmap9->set_alignment(0.5, 0.5);
   pixmap9->set_padding(0, 0);
   viewport20->set_shadow_type(GTK_SHADOW_NONE);
   viewport20->add(*waffenbesitz_alt_tree);
   scrolledwindow56->set_usize(300, 260);
   scrolledwindow56->set_policy(GTK_POLICY_AUTOMATIC, GTK_POLICY_AUTOMATIC);
   scrolledwindow56->add(*viewport20);
   pixmap141->set_alignment(0.5, 0.5);
   pixmap141->set_padding(0, 0);
   label561->set_alignment(0.5, 0.5);
   label561->set_padding(0, 0);
   hbox65->pack_start(*pixmap141);
   hbox65->pack_start(*label561, false, false, 0);
   checkbutton_mag_waffenbonus->set_flags(GTK_CAN_FOCUS);
   _tooltips.set_tip(*checkbutton_mag_waffenbonus, "Nach dem aktivieren: Waffe wählen", "");
   checkbutton_mag_waffenbonus->set_mode(false);
   checkbutton_mag_waffenbonus->add(*hbox65);
   table70->attach(*checkbutton_mag_waffenbonus, 0, 1, 0, 1, GTK_FILL, 0, 0, 0);
   viewport21->set_shadow_type(GTK_SHADOW_NONE);
   viewport21->add(*waffenbesitz_neu_tree);
   scrolledwindow57->set_usize(300, 260);
   scrolledwindow57->set_policy(GTK_POLICY_AUTOMATIC, GTK_POLICY_AUTOMATIC);
   scrolledwindow57->add(*viewport21);
   spinbutton_av_bonus->set_editable(true);
   spinbutton_sl_bonus->set_editable(true);
   label_waffe->set_alignment(0, 0.5);
   label_waffe->set_padding(0, 0);
   label_SL_bonus->set_alignment(0, 0.5);
   label_SL_bonus->set_padding(0, 0);
   label_av_bonus->set_alignment(0, 0.5);
   label_av_bonus->set_padding(0, 0);
   label395->set_alignment(0, 0.5);
   label395->set_padding(0, 0);
   _tooltips.set_tip(*entry_magisch, "Für eine magische (+0/+0) Waffe muß hier ein '*' eingetragen werden.", "");
   entry_magisch->set_editable(true);
   table62->attach(*label395, 0, 1, 0, 1, GTK_FILL, 0, 5, 0);
   table62->attach(*entry_magisch, 1, 2, 0, 1, GTK_EXPAND|GTK_FILL, 0, 0, 0);
   table_magbonus->attach(*spinbutton_av_bonus, 1, 2, 1, 2, GTK_EXPAND|GTK_FILL, 0, 0, 0);
   table_magbonus->attach(*spinbutton_sl_bonus, 1, 2, 2, 3, GTK_EXPAND|GTK_FILL, 0, 0, 0);
   table_magbonus->attach(*label_waffe, 0, 2, 0, 1, 0, 0, 0, 3);
   table_magbonus->attach(*label_SL_bonus, 0, 1, 2, 3, GTK_FILL, 0, 5, 0);
   table_magbonus->attach(*label_av_bonus, 0, 1, 1, 2, GTK_FILL, 0, 5, 0);
   table_magbonus->attach(*table62, 0, 2, 3, 4, GTK_FILL, GTK_EXPAND|GTK_FILL, 0, 0);
   pixmap155->set_alignment(0.5, 0.5);
   pixmap155->set_padding(0, 0);
   label581->set_alignment(0.5, 0.5);
   label581->set_padding(0, 0);
   hbox75->pack_start(*pixmap155);
   hbox75->pack_start(*label581, false, false, 0);
   button58->add(*hbox75);
   table43->attach(*label315, 0, 1, 0, 1, 0, 0, 0, 3);
   table43->attach(*label316, 2, 3, 0, 1, GTK_EXPAND, 0, 0, 3);
   table43->attach(*pixmap9, 1, 2, 0, 1, GTK_EXPAND|GTK_FILL, GTK_FILL, 0, 0);
   table43->attach(*scrolledwindow56, 0, 1, 1, 2, GTK_EXPAND|GTK_FILL, GTK_EXPAND|GTK_FILL, 0, 0);
   table43->attach(*table70, 0, 1, 3, 4, GTK_FILL, GTK_FILL, 0, 0);
   table43->attach(*scrolledwindow57, 2, 3, 1, 3, GTK_EXPAND|GTK_FILL, GTK_EXPAND|GTK_FILL, 0, 0);
   table43->attach(*table_magbonus, 0, 1, 2, 3, GTK_FILL, GTK_FILL, 0, 0);
   table43->attach(*button58, 2, 3, 3, 4, 0, 0, 0, 0);
   Window_Waffenbesitz->set_title("Waffenbesitz");
   Window_Waffenbesitz->set_modal(true);
   Window_Waffenbesitz->set_policy(false, true, true);
   Window_Waffenbesitz->add_accel_group(*(_data->getAccelGroup()));
   Window_Waffenbesitz->add(*table43);
   label315->show();
   label316->show();
   pixmap9->show();
   waffenbesitz_alt_tree->show();
   viewport20->show();
   scrolledwindow56->show();
   pixmap141->show();
   label561->show();
   hbox65->show();
   checkbutton_mag_waffenbonus->show();
   table70->show();
   waffenbesitz_neu_tree->show();
   viewport21->show();
   scrolledwindow57->show();
   spinbutton_av_bonus->show();
   spinbutton_sl_bonus->show();
   label_waffe->show();
   label_SL_bonus->show();
   label_av_bonus->show();
   label395->show();
   entry_magisch->show();
   table62->show();
   table_magbonus->show();
   pixmap155->show();
   label581->show();
   hbox75->show();
   button58->show();
   table43->show();
   Window_Waffenbesitz->show();
   waffenbesitz_alt_tree->leaf_selected.connect(SigC::slot(static_cast<class Window_Waffenbesitz*>(this), &Window_Waffenbesitz::on_leaf_selected_alt));
   waffenbesitz_alt_tree->reorder.connect(SigC::slot(static_cast<class Window_Waffenbesitz*>(this), &Window_Waffenbesitz::on_alte_waffen_reorder));
   checkbutton_mag_waffenbonus->toggled.connect(SigC::slot(static_cast<class Window_Waffenbesitz*>(this), &Window_Waffenbesitz::on_checkbutton_mag_waffenbonus_toggled));
   waffenbesitz_neu_tree->leaf_selected.connect(SigC::slot(static_cast<class Window_Waffenbesitz*>(this), &Window_Waffenbesitz::on_leaf_selected_neu));
   spinbutton_av_bonus->activate.connect(SigC::slot(static_cast<class Window_Waffenbesitz*>(this), &Window_Waffenbesitz::on_spinbutton_av_bonus_activate));
   spinbutton_sl_bonus->activate.connect(SigC::slot(static_cast<class Window_Waffenbesitz*>(this), &Window_Waffenbesitz::on_spinbutton_sl_bonus_activate));
   entry_magisch->activate.connect(SigC::slot(static_cast<class Window_Waffenbesitz*>(this), &Window_Waffenbesitz::on_entry_magisch_activate));
   button58->clicked.connect(SigC::slot(static_cast<class Window_Waffenbesitz*>(this), &Window_Waffenbesitz::on_button_close_clicked));
}

Window_Waffenbesitz_glade::~Window_Waffenbesitz_glade()
{  delete waffenbesitz_alt_tree;
   delete waffenbesitz_neu_tree;
   delete _data;
}
