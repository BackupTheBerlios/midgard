  ...
  for(std::vector<Sprache_und_Schrift>::const_iterator i=L.begin();i!=L.end();++i)
   {
      ...
      for(std::vector<Sprache_und_Schrift::st_sus>::const_iterator j=i->getSchriften().begin();j!=i->getSchriften().end();)
       {
         ss+= TeX::string2TeX(j->buchstaben) + "$\\;$+"+itos(j->wert);
         if(++j!=i->getSchriften().end())  ss+=", ";
       }
      if(!longlist) fout << "\\newcommand{\\schr"<<a<<"}";
      else fout << " & ";
      fout <<"{\\scriptsize "<< TeX::scale(ss,20,"2.6cm",false) <<"}";
      if(longlist) fout << "\\\\";
      fout << "\n";
   }
 if(!longlist)
  ...
  for(std::list<H_WaffeBesitz>::const_iterator i=L.begin();i!=L.end();++i)
   {
     ...
     VWB.push_back(st_WB(LaTeX_scalemag(waffenname,25,"3cm",(*i)->Magisch(),
            (*i)->Waffe()->Reichweite()+" "+(*i)->Waffe()->Text()),
 ...
 for(std::list<Abenteurer::st_universell>::iterator i=UF.begin();i!=UF.end();++i)
  {
    ...
    if (!i->gelernt)
     {
       ++countunifert;
       fout <<"\\newcommand{\\uni"<<a<<"}{"<<TeX::string2TeX(name)<< "}\t\t";
       if (i->voraussetzung)
       ...
       const std::string& magisch,const std::string& reichweite)
{
 ...
 if (is.size() <= maxlength) os = TeX::string2TeX(is);
  ...
  if (magisch==""||magisch=="*") 
   {
     if(reichweite.size()<=25) l2 = "{\\tiny "+TeX::string2TeX(reichweite)+"}";
     else l2 = "\\resizebox*{"+scale+"}{1.5ex}{\\tiny "+TeX::string2TeX(reichweite)+"}" ;
   }
  else  
    { if (magisch.size()<=25) l2 = "\\tiny "+TeX::string2TeX(magisch);
      else l2 = "\\resizebox*{"+scale+"}{1.5ex}{\\tiny "+TeX::string2TeX(magisch)+"}" ;
    }
