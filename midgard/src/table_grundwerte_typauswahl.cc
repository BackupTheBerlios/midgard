...
void table_grundwerte::typauswahl_button()
{
 ...
 hauptfenster->getChar().getWizard().done(Wizard::TYP,*(hauptfenster->getChar()));
...
// hauptfenster->getChar().undosave("Spezies gewählt");
 ...
 hauptfenster->getChar().getWizard().done(Wizard::SPEZIES,*(hauptfenster->getChar()));
}
...
void table_grundwerte::on_radiobutton_stadt_land_toggled()
{
  ...
  hauptfenster->getChar().getWizard().done(Wizard::STADTLAND,*(hauptfenster->getChar()));
...
void table_grundwerte::on_radiobutton_mann_toggled()
{
  ManuProC::Trace _t(table_grundwerte::trace_channel,__FUNCTION__);
  if(block_changed) return;
  hauptfenster->getChar().getWizard().done(Wizard::GESCHLECHT,*(hauptfenster->getChar()));
