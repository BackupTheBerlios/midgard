...
void midgard_CG::on_abge_werte_setzen_clicked()
{
  ...
  if (Typ[0]->Sb() == 20 || Typ[1]->Sb() ) sb += random.integer(1,20); 
  // Spitzbube
  if (Typ[0]->Sb() == -20&& Typ[1]->Sb() == -20) sb -= random.integer(1,20); 
  // Saddhu
  ...
  int zaubern_wert=0;
  if (Typ[0]->Zaubern() == "j" || Typ[0]->Zaubern() == "z" || Typ[1]->Zaubern() == "j" || Typ[1]->Zaubern() == "z" ) zaubern_wert = 10 ;
