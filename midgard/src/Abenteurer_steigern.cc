   ...
   if(bool_steigern.aep_fuellen)
    {
      ...
      if(ep_k > use_pp*40)
       {
         info+="H�chstens die H�lfte der GFP darf beim 'Steigern mit PP' durch die H�lfte der GFP darf beim 'Steigern mit PP' durch EP bestritten werden"; 
         return false;
       }
    }  
   else
...
//???kido steigern und nicht neu lernen     if ((*MBE)->Name()=="KiDo" && kido_steigern_check(MBE->Erfolgswert())) return;
   ...
   { MyList     = &List_Sprache();
     // eventuell h�herer Erfolgswert weil die Sprache schon ungelernt beherrscht wird)
...
void Abenteurer::eigenschaften_steigern(std::string &info,const Datenbank &Database,int wurf)
{
  if(!eigenschaften_steigern_erlaubt())
   {info+="F�r Grad "+itos(getWerte().get_Grad_Basiswerte())+" wurde schon gew�rfelt";
    return;
   }
  // Erh�hen der Schicksalsgunst
  ...
  int z=wurf;  
  info+="Beim W�rfeln zur Erh�hung einer Eigenschaft f�r Grad "
