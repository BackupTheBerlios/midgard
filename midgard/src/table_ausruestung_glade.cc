...
{  table_ausruestung = this;
   ...
   Gtk::Viewport *viewport48 = manage(new class Gtk::Viewport());
   Gtk::ScrolledWindow *scrolledwindow87 = manage(new class Gtk::ScrolledWindow());
   Gtk::Table *table93 = manage(new class Gtk::Table(3, 4, false));
   ...
   viewport48->set_shadow_type(GTK_SHADOW_NONE);
   viewport48->add(*preise_tree_neu);
   scrolledwindow87->set_policy(GTK_POLICY_AUTOMATIC, GTK_POLICY_AUTOMATIC);
   scrolledwindow87->add(*viewport48);
   table93->set_row_spacings(0);
   table93->set_col_spacings(0);
   table93->attach(*scrolledwindow64, 0, 1, 2, 3, GTK_SHRINK|GTK_FILL, GTK_SHRINK|GTK_FILL, 0, 0);
   table93->attach(*hpaned4, 0, 4, 1, 2, GTK_EXPAND|GTK_SHRINK|GTK_FILL, GTK_EXPAND|GTK_SHRINK|GTK_FILL, 0, 0);
   table93->attach(*table92, 2, 3, 2, 3, GTK_SHRINK, GTK_SHRINK, 5, 4);
   table93->attach(*button_modi, 1, 2, 2, 3, GTK_SHRINK, GTK_SHRINK, 5, 0);
   table93->attach(*scrolledwindow87, 0, 2, 0, 1, GTK_EXPAND|GTK_SHRINK|GTK_FILL, GTK_EXPAND|GTK_SHRINK|GTK_FILL, 0, 0);
