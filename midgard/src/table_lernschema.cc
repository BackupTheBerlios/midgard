...
void table_lernschema::show_gelerntes()
{
  ...
  for(std::list<std::list<MBEmlt> >::const_iterator i=LL.begin();i!=LL.end();++i)
    for (std::list<MBEmlt>::const_iterator j=i->begin();j!=i->end();++j)
      FL.push_back(*j);
  MidgardBasicTree::show_list_in_tree(FL,tree_gelerntes,hauptfenster->getChar().actualIterator());
...
void table_lernschema::show_lernschema()
{
  ...
  MidgardBasicTree::show_list_in_tree(newlist,tree_lernschema,hauptfenster->getChar().actualIterator());
