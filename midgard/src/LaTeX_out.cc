...
gint midgard_CG::on_latex_release_event(GdkEventButton *ev)
{
  ...
  if (ev->button==2) latex_beschreibung_drucken();
  if (ev->button==3) on_ausruestung_druck(true);
  return false;
}
 ...
 for(std::list<cH_MidgardBasicElement>::iterator i=UF.begin();i!=UF.end();++i)
  {
    ...
    if ((*i)->What()==MidgardBasicElement::FERTIGKEIT) 
      { cH_Fertigkeit f(*i);
        ...
        if (!f->Voraussetzungen(Werte,list_Fertigkeit)) {iwert-=2; voraussetzung=false;}
      ...
      { cH_Waffe f(*i);
        ...
        if (!f->SG_Voraussetzung(Werte,list_Fertigkeit,list_Waffen)) {iwert=0; voraussetzung=false;}
        if   (iwert>=0) wert = "+"+itos(iwert);
        else            wert = "--"+itos(abs(iwert));
      }
