...
/*
double cH_Zauber::Faktor(std::list<cH_Zauber>& LZ,const std::string& n,const vector<Data_typen>& Typen,const Grundwerte& Werte) 
{
 for(std::list<cH_Zauber>::const_iterator i=LZ.begin();i!=LZ.end();++i)
   if (n==(*i)->Name())
     {
       double fac;
       if (Typen[0].Short() != "eBe" || ((*i)->P_Element()=="" && (*i)->S_Element()==""))
        {
         if      ((*i)->Standard()=="G" || (*i)->Standard2()=="G") fac = 0.5;
         else if ((*i)->Standard()=="S" && ( (*i)->Standard2()=="S" || (*i)->Standard2()=="" )) fac = 1.0;
         else if ((*i)->Standard2()=="S" && ( (*i)->Standard()=="S" || (*i)->Standard()=="" )) fac = 1.0; 
         else fac = 5.0; 
        }
      else
        {
         if ((*i)->P_Element()==Werte.Spezial())  fac = 0.5;
         if ((*i)->S_Element()==Werte.Spezial2()) fac = 1;  
         else fac = 0;
       }
      return(fac);
     }
...
void cH_Zauber::set_Kosten(std::list<cH_Zauber>& LZ,const std::string& n,int k)
{
 for(std::list<cH_Zauber>::const_iterator i=LZ.begin();i!=LZ.end();++i)
   if (n==(*i)->Name())
     {(*i)->set_Kosten(k);break;}
}
void cH_Zauber::set_Standard(std::list<cH_Zauber>& LZ,const std::string& n,const std::string& s, const std::string& s2)
{
 for(std::list<cH_Zauber>::const_iterator i=LZ.begin();i!=LZ.end();++i)
   if (n==(*i)->Name())
     {(*i)->set_Standard(s);(*i)->set_Standard2(s2); break;}
}
*/