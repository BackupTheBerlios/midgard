...
{  
  ...
  switch(MBE->What()) {
           ...
           if(togglebutton_spezialwaffe->get_active() )
            { 
              ...
              {  
               ...
               Waffe::setSpezialWaffe(hauptfenster->getCWerte().Spezialisierung(),hauptfenster->list_Waffen);
            ...
            {  
              hauptfenster->list_Waffen.remove(MBE);
              lernpunkte.addWaffen(MBE->Lernpunkte());
            }
           break;
         }
     case MidgardBasicElement::WAFFEBESITZ :
         { hauptfenster->list_Waffen_besitz.remove(MBE);  
     ...
     case MidgardBasicElement::SPRACHE :
           hauptfenster->list_Sprache.remove(MBE);
           // KEIN break 
     case MidgardBasicElement::SCHRIFT :
           hauptfenster->list_Schrift.remove(MBE);
  ...
  switch(MBE->What()) {
    case MidgardBasicElement::WAFFE:
      { 
        ...
        hauptfenster->list_Waffen.push_back(MBE);
    ...
    case MidgardBasicElement::ZAUBER:
      { 
        ...
        if(!MBE->ZusatzEnum(hauptfenster->Typ)) // Das macht 'lernen_zusatz' automatisch
            hauptfenster->list_Zauber.push_back(MBE); 
        else lernen_zusatz(MBE->ZusatzEnum(hauptfenster->Typ),MBE);
        lernpunkte.addZauber(- MBE->Lernpunkte());   
        break; }
    case MidgardBasicElement::FERTIGKEIT:
      { 
        ...
        if(!SpracheSchrift(MBE))
          { 
            ...
            if(!MBE->ZusatzEnum(hauptfenster->Typ) && st==std::string::npos)// Das macht 'lernen_zusatz' automatisch
                  hauptfenster->list_Fertigkeit.push_back(MBE); 
            if(MBE->Name()=="KiDo" && hauptfenster->Typ[0]->Short()=="Kd") maxkido+=2;
            if(maxkido>0 && cH_Fertigkeit("KiDo")->ist_gelernt(hauptfenster->list_Fertigkeit))
               zeige_werte();
         }
        else
...
//  if(MBE->ZusatzEnum(hauptfenster->Typ)) scrolledwindow_lernen->set_sensitive(false);
}
...
{  
  ...
  int lpspezies=0;
  hauptfenster->list_Fertigkeit=hauptfenster->getCWerte().Spezies()->getFertigkeiten(lpspezies,hauptfenster->getCWerte());
  show_gelerntes();

  int fachlern=hauptfenster->random.integer(1,6)+hauptfenster->random.integer(1,6);
  if(!hauptfenster->list_Fertigkeit.empty())
    hauptfenster->set_status("Die Lernpunkte ("+itos(lpspezies)+") für die Pflichtfertigkeiten"
      " für die Spezies "+hauptfenster->getCWerte().Spezies()->Name()+" wurden von den erfürfelten"
  ...
  if (hauptfenster->Typ[1]->Short()=="") lernpunkte.setWaffen(hauptfenster->random.integer(1,6)+hauptfenster->random.integer(1,6));
  else                     lernpunkte.setWaffen(hauptfenster->random.integer(1,6)+1); // Doppelcharakter
  if (hauptfenster->Typ[0]->is_mage() && hauptfenster->Typ[1]->Short()=="")
      lernpunkte.setZauber(hauptfenster->random.integer(1,6)+hauptfenster->random.integer(1,6));
  if (hauptfenster->Typ[1]->is_mage() && hauptfenster->Typ[1]->Short()!="")
...
//  spinbutton_alter->set_value(hauptfenster->getCWerte().Alter());
  ...
  if(hauptfenster->Typ[1]->is_mage() || hauptfenster->Typ[1]->is_mage() ) //|| magie_bool)
...
void table_lernschema::lernschema_geld_wuerfeln()
{
 ...
 igold=V[0]+V[1]+V[2];
 if      (hauptfenster->Typ[0]->Geld() == 1) igold-=3;
 else if (hauptfenster->Typ[0]->Geld() == 2) igold+=0;
 else if (hauptfenster->Typ[0]->Geld() == 3) igold+=6;
 else if (hauptfenster->Typ[0]->Geld() == 4) igold+=3;
...
void table_lernschema::on_button_ruestung_clicked()
{
  ...
  if (hauptfenster->Typ[0]->Ruestung() == 1)
   {
  ...
  if (hauptfenster->Typ[0]->Ruestung()== 2)
   {
      if ( 1 <= wurf && wurf  <= 10 ) rue = "OR" ;
      if (11 <= wurf && wurf  <= 20 ) rue = "TR" ;
  ...
  if (hauptfenster->Typ[0]->Ruestung()== 3)
   {
      if ( 1 <= wurf && wurf  <= 10 ) rue = "OR" ;
      if (11 <= wurf && wurf  <= 20 ) rue = "TR" ;
      if (21 <= wurf && wurf  <= 30 ) rue = "LR" ;
  ...
  if (hauptfenster->Typ[0]->Ruestung()== 4)
   {
      if ( 1 <= wurf && wurf  <= 10 ) rue = "OR" ;
...
void table_lernschema::zeige_werte()
{
  ...
  fertig_typ->set_text(hauptfenster->Typ[0]->Name(hauptfenster->getCWerte().Geschlecht()));
  if (hauptfenster->Typ[1]->Name(hauptfenster->getCWerte().Geschlecht())!="")
   fertig_typ->set_text(hauptfenster->Typ[0]->Name(hauptfenster->getCWerte().Geschlecht())+"/"+hauptfenster->Typ[1]->Name(hauptfenster->getCWerte().Geschlecht()));

  // Spezialwaffe anzeigen?
 if (hauptfenster->Typ[0]->Spezialwaffe() || hauptfenster->Typ[1]->Spezialwaffe())
 ...
 // Magie anzeigen?
 if (hauptfenster->Typ[0]->is_mage() || hauptfenster->Typ[1]->is_mage() )//|| magie_bool)
   { if ( hauptfenster->Typ[0]->Spezialgebiet() || 
          hauptfenster->Typ[1]->Spezialgebiet() ) 
 ...
 // KiDo anzeigen?
 if(cH_Fertigkeit("KiDo")->ist_gelernt(hauptfenster->list_Fertigkeit))
   { 
...
void table_lernschema::show_gelerntes()
{
  ...
  LL.push_back(hauptfenster->list_Fertigkeit_ang);
  LL.push_back(hauptfenster->list_Fertigkeit);
  LL.push_back(hauptfenster->list_Waffen);
  LL.push_back(hauptfenster->list_Zauber);
  LL.push_back(hauptfenster->list_Zauberwerk);
  LL.push_back(hauptfenster->list_Kido);
  LL.push_back(hauptfenster->list_WaffenGrund);
  LL.push_back(hauptfenster->list_Waffen_besitz);
  LL.push_back(hauptfenster->list_Sprache);
  LL.push_back(hauptfenster->list_Schrift);
  LL.push_back(hauptfenster->list_Beruf);  
    ...
    for(std::list<cH_MidgardBasicElement>::const_iterator i=hauptfenster->getDatabase().Fertigkeit.begin();i!=hauptfenster->getDatabase().Fertigkeit.end();++i)
     {
      ...
      if(!(*i)->ist_gelernt(hauptfenster->list_Fertigkeit))
        {
      ...
      if ((*i)->ist_gelernt(hauptfenster->list_Fertigkeit)) (*i)->setGelernt(true); 
...
//<<(*i)->ist_gelernt(hauptfenster->list_Fertigkeit)<<'\n';
      newlist.push_back(*i);
     }
   }
  ...
  if(fert!="Unge" && fert!="Allg" )
    {
      if(what==MidgardBasicElement::WAFFE)
         LW=hauptfenster->getDatabase().lernschema.get_List("Waffenfertigkeiten",hauptfenster->Typ,hauptfenster->list_Waffen);
      if(what==MidgardBasicElement::ZAUBER)
         LW=hauptfenster->getDatabase().lernschema.get_List("Zauberkünste",hauptfenster->Typ,hauptfenster->list_Zauber);
      if(what==MidgardBasicElement::FERTIGKEIT)
         LW=hauptfenster->getDatabase().lernschema.get_List("Fachkenntnisse",hauptfenster->Typ,hauptfenster->list_Fertigkeit);
      for(std::list<cH_MidgardBasicElement>::const_iterator i=LW.begin();i!=LW.end();++i)
        {
          bool gelernt=false;
...
//             if ((*i)->ist_gelernt(list_FertigkeitZusaetze)) gelernt=true;
           ...
           else if(what==MidgardBasicElement::FERTIGKEIT)
            {
             ...
             if (!cH_Fertigkeit(*i)->Voraussetzungen(hauptfenster->getCWerte(),hauptfenster->list_Fertigkeit)) continue ;
             if(hauptfenster->getCWerte().Spezies()->istVerbotenSpielbegin(*i)) continue;
             if ((*i)->ist_gelernt(hauptfenster->list_Fertigkeit)) gelernt=true;   
             if ((*i)->ist_gelernt(list_FertigkeitZusaetze)) gelernt=true;
             VI=Lernschema::getIndex(hauptfenster->Typ,"Fachkenntnisse",(*i)->Name());  
