...
{  //libmagus_init0magus_paths::init(argv[0]);
 try {  
   ...
   MBEmlt ff=ch->getAbenteurer().get_unknown(cH_Sprache("Ffomorisch"));
   ch->getAbenteurer().Erlernen(ff);
   ch->name_undo("Ffomorisch mit PP erlernt");
   { std::ofstream out("test5.magus");
