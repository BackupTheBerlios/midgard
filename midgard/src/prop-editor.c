...
static void brother_cb(GtkWidget *button,GObject *object)
{  assert(GTK_IS_WIDGET(object) && GTK_WIDGET(object)->parent);
   GList *children = gtk_container_get_children (GTK_CONTAINER (GTK_WIDGET(object)->parent));
   // ...
   while (children)
   {  if (children->data==object && children->next && children->next->data)
         create_prop_editor(G_OBJECT(children->next->data),0);
      children = children->next;
   }
   g_list_free(children);
}

static void child_cb(GtkWidget *button,GObject *object)
{  assert(GTK_IS_CONTAINER(object));
		    ...
		    GType      type)
{
  ...
  if (type == 0)
    { GtkWidget *vbox = gtk_vbox_new(FALSE,0);
      GtkWidget *b_brother=0,*b_child=0;
      ...
      b_brother=gtk_button_new_with_label("brother");
      g_signal_connect(b_brother,"clicked", G_CALLBACK(brother_cb),object);
      gtk_widget_set_sensitive(b_brother, GTK_IS_WIDGET(object) && GTK_WIDGET(object)->parent);
      gtk_container_add(GTK_CONTAINER(vbox), b_brother);
      b_child=gtk_button_new_with_label("child");
      g_signal_connect(b_child,"clicked", G_CALLBACK(child_cb),object);
      gtk_widget_set_sensitive(b_child, GTK_IS_CONTAINER(object));
      gtk_container_add(GTK_CONTAINER(vbox), b_child);
