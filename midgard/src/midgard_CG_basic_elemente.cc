...
/*
  if(mbe.begin()->What()==MidgardBasicElement::FERTIGKEIT)
   {
    list_Fertigkeiten = mbe;
    maxkido=0;  if (Typ[0]->Short()=="Kd") maxkido=2;
    int KD_tech=0;
    for(std::list<cH_MidgardBasicElement>::iterator i=list_Fertigkeiten.begin();i!=list_Fertigkeiten.end();++i)
      {
         cH_Fertigkeit f(*i);
    ...
    for (int j=0;j<KD_tech;++j)
      for(std::list<cH_MidgardBasicElement>::iterator i=list_Fertigkeiten.begin();i!=list_Fertigkeiten.end();++i)
       if (cH_Fertigkeit(*i)->Name()=="KiDo-Technik") {list_Fertigkeiten.erase(i);break;}
    show_fertigkeiten();
