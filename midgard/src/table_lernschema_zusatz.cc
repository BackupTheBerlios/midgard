...
//  MBE_mem=&MBE;
  ...
  // Weil Fertigkeiten mehrmals gelernt werden d¸rfen werde sie hier nicht 
...
// list_FertigkeitZusaetze.push_back(MBE->Name());
  if(MBE.Lernpunkte()==0) // Sprache/Schrift f¸r '0' Lernpunkte nur einmal lernen
    list_FertigkeitZusaetze.push_back(MBE->Name());
     ...
     case MidgardBasicElement::ZSprache:
      {
       for (std::list<cH_MidgardBasicElement>::const_iterator i=hauptfenster->getDatabase().Sprache.begin();i!=hauptfenster->getDatabase().Sprache.end();++i)
         {
            bool erlaubt=true;
            if(MBE->Name()=="Muttersprache") // muﬂ im Heimatland gesprochen werden
             {
     ...
     case MidgardBasicElement::ZWaffe:
      {
       for (std::list<MidgardBasicElement_mutable>::const_iterator i=hauptfenster->getChar().List_Waffen().begin();i!=hauptfenster->getChar().List_Waffen().end();++i)
        if (cH_Waffe(*i)->Art()=="Schuﬂwaffe" || cH_Waffe(*i)->Art()=="Wurfwaffe")
          datavec_zusatz.push_back(new Data_Zusatz(MBE,(*i)->Name(),true));
       if(datavec_zusatz.empty()) 
         { hauptfenster->set_status("Noch keine Fernkampfwaffe gew‰hlt.");
  ...
  switch(was)
   {
     case MidgardBasicElement::ZHerkunft :
      {
       frame_lernschema_zusatz->set_label("Herkunftsland w‰hlen");
       vs.push_back("Land");
     ...
     case MidgardBasicElement::ZWaffe :
      {
       frame_lernschema_zusatz->set_label("Fernkampfwaffe ausw‰hlen");
     ...
     case MidgardBasicElement::ZTabelle :
      {
       frame_lernschema_zusatz->set_label("Zusatz ausw‰hlen");
       vs.push_back(MBE->Name());
       vs.push_back("");
       vs.push_back("");
       break;
     ...
     case MidgardBasicElement::ZSchrift :
      {
       frame_lernschema_zusatz->set_label("Schrift ausw‰hlen");
       vs.push_back(MBE->Name());
       vs.push_back("");
       vs.push_back("");
       break;
     ...
     case MidgardBasicElement::ZSprache :
      {
       ...
       else frame_lernschema_zusatz->set_label("Sprache ausw‰hlen");
       vs.push_back(MBE->Name());
       vs.push_back("");
       vs.push_back("");
       break;
