...
void midgard_CG::on_waffen_laden_clicked()
{
 ...
 for (std::list<cH_MidgardBasicElement>::const_iterator i=Database.Waffe.begin();i!=Database.Waffe.end();++i)
   { cH_Waffe w(*i);
     if ((*i)->ist_gelernt(Char.CList_Waffen())) continue ;
     if(getCWerte().Spezies()->istVerboten(*i)) continue;
     if (!w->Grundkenntnis_vorhanden(Char.CList_WaffenGrund())) continue;
     if (!w->ist_lernbar(Char.getVTyp(),w->get_MapTyp())) continue;
     if (!region_check(w->Region(w->Name())) ) continue;
     if (w->SG_Voraussetzung(getCWerte(),Char.CList_Fertigkeit(),Char.List_Waffen()))
...
void midgard_CG::waffen_zeigen()
{
   ...
   MidgardBasicElement::show_list_in_tree(Char.CList_WaffenGrund()    ,alte_grund_tree,this);
   MidgardBasicElement::show_list_in_tree(list_Waffen_neu,neue_waffen_tree,this);
   MidgardBasicElement::show_list_in_tree(Char.CList_Waffen()    ,alte_waffen_tree,this);
}
...
void midgard_CG::on_waffengrund_laden_clicked()
{
  ...
  for (std::list<cH_MidgardBasicElement>::const_iterator i=Database.WaffeGrund.begin();i!=Database.WaffeGrund.end();++i)
   { cH_WaffeGrund w(*i);
     if ((*i)->ist_gelernt(Char.CList_WaffenGrund())) continue ;
     if(getCWerte().Spezies()->istVerboten(*i)) continue;
     if((*i)->ist_lernbar(Char.getVTyp(),(*i)->get_MapTyp()))
...
void midgard_CG::on_alte_waffen_reorder()
{
  ...
  switch((Data_SimpleTree::Spalten_LONG_ALT)seq[0]) {
      case Data_SimpleTree::WERTa : Char.List_Waffen().sort(cH_MidgardBasicElement::sort(cH_MidgardBasicElement::sort::ERFOLGSWERT)); ;break;
      case Data_SimpleTree::NAMEa : Char.List_Waffen().sort(cH_MidgardBasicElement::sort(cH_MidgardBasicElement::sort::NAME)); ;break;
