     ...
     for(std::map<std::string,std::vector<PreiseNewMod::st_preismod> >::const_iterator j=VM.begin();j!=VM.end();++j)     
      {
        ...
        for(std::vector<PreiseNewMod::st_preismod>::const_iterator k=j->second.begin();k!=j->second.end();++k)
         {     
//           MS[spalte.first].push_back(PreiseNewMod::st_preismod(k->spezifikation, k->preis_faktor));
           MS[spalte.first].push_back(*k);
         }
      }
     std::map<table_ausruestung::e_spalten,vector<std::string> > M;
     for(std::map<e_spalten,std::vector<PreiseNewMod::st_preismod> >::const_iterator j=MS.begin();j!=MS.end();++j)
      {
        std::map<table_ausruestung::e_spalten,PreiseNewMod::st_preismod> X;
        for(std::vector<PreiseNewMod::st_preismod>::const_iterator k=j->second.begin();k!=j->second.end();++k)
         {
           X[j->first]=*k ;
           datavec.push_back(new Data_NewPreis(*i,X));
         }
      }
   }
