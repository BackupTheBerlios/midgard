...
#include "test_header.cc"
   ...
   ch->begin_undo();
   ch->getAbenteurer().wie_steigern=Abenteurer::ws_Spruchrolle;
   ch->getAbenteurer().wie_steigern_variante=Abenteurer::wsv_SpruchrolleAlways;
   MBEmlt zau=ch->getAbenteurer().get_unknown(cH_Zauber("Reinigen"));
   ch->getAbenteurer().Erlernen(zau);
   ch->name_undo("Zauber mit Spruchrolle erlernen");
   { std::ofstream out("test8.magus");
