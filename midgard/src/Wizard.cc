...
//  if(step==LERNSCHEMA_SEITE) { next_step(LERNSCHEMA_SEITE); return;}
  ...
  if(step==SPEZIALWAFFE&&(!hauptfenster->getAben().Typ1()->Spezialwaffe()&&!hauptfenster->getAben().Typ2()->Spezialwaffe()))
   {
     next_step(SPEZIALWAFFE);
     return;
   }
  if(step==SPEZIALGEBIET&&(!hauptfenster->getAben().Typ1()->Spezialgebiet()&&!hauptfenster->getAben().Typ2()->Spezialgebiet()))
   {
